...
<body class="bg-gray-50 overflow-x-hidden">
    ...
    <div x-data="{ open: false }" class="fixed bottom-24 right-5 z-50 flex flex-col items-center gap-3">
        ...
        <a href="tel:{{ preg_replace('/[^0-9+]/', '', $callPhone) }}"
           ...
           class="w-14 h-14 bg-blue-500 hover:bg-blue-600 text-white rounded-full shadow-lg flex items-center justify-center transition-all hover:scale-110"
           style="display:none;">
        ...
        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $waPhone) }}" target="_blank" rel="noopener"
           ...
           x-transition:leave-start="opacity-100 translate-y-0 scale-100"
           x-transition:leave-end="opacity-0 translate-y-4 scale-75"
           class="w-14 h-14 bg-[#25D366] hover:bg-[#20b858] text-white rounded-full shadow-lg flex items-center justify-center transition-all hover:scale-110 relative"
           style="display:none;">
            ...
            <!-- Online dot -->
            <span class="absolute top-0.5 right-0.5 w-3 h-3 bg-green-300 rounded-full border-2 border-white"></span>
        </a>
        @endif

        <!-- Toggle Button (red X / green chat) -->
        <button @click="open = !open"
                class="w-14 h-14 rounded-full shadow-xl flex items-center justify-center transition-all duration-300 hover:scale-110"
                :class="open ? 'bg-red-500 hover:bg-red-600' : 'bg-[#25D366] hover:bg-[#20b858]'">
            <i class="fas text-white text-xl transition-all duration-300"
