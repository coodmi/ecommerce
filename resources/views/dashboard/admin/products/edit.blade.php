                    ...
                    <div class="space-y-4">
                        ...
                        <div class="sm:col-span-2">
                            <label class="block text-sm font-semibold text-gray-700 mb-3">
                                Delivery Charge per Zone
                                <span class="text-gray-400 font-normal text-xs ml-1">— leave blank to use zone's default</span>
                            </label>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                @foreach($deliveryZones as $zone)
                                @php
                                    $zoneCharges = $product->zone_charges ?? [];
                                    $savedCharge = $zoneCharges[$zone->id] ?? null;
                                @endphp
                                <div class="border border-gray-200 rounded-xl p-4">
                                    <div class="flex items-center gap-2 mb-2">
                                        <i class="fas {{ $zone->icon }} text-primary text-sm"></i>
                                        <span class="text-sm font-semibold text-gray-800">{{ $zone->name }}</span>
                                        <span class="text-xs text-gray-400 ml-auto">Default: ৳{{ number_format($zone->charge, 0) }}</span>
                                    </div>
                                    <div class="relative">
                                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">৳</span>
                                        <input type="number"
                                               name="zone_charges[{{ $zone->id }}]"
                                               value="{{ old('zone_charges.' . $zone->id, $savedCharge) }}"
                                               min="0" step="1"
                                               class="w-full pl-7 pr-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-primary transition"
                                               placeholder="{{ number_format($zone->charge, 0) }}">
                                    </div>
                                </div>
                                @endforeach
                            </div>
                            <p class="text-xs text-gray-400 mt-2">Leave blank to use the zone's default charge</p>
                        </div>
                    </div>
                </div>
