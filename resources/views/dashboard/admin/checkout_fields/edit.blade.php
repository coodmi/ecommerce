...
@section('title', 'Edit Checkout Field')
...
<div class="p-6">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-display font-bold text-gray-900">Edit Checkout Field</h1>
        <p class="text-gray-600 mt-1">Update field configuration for the checkout form</p>
    </div>
            ...
            <div class="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 p-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="label" required value="{{ old('label', $checkoutField->label) }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" required value="{{ old('name', $checkoutField->name) }}"
                               class="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-indigo-500 focus:bg-white transition-all outline-none text-slate-900 font-medium"
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="type" required
                                class="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-indigo-500 focus:bg-white transition-all outline-none text-slate-900 font-medium">
                            <option value="text" {{ $checkoutField->type == 'text' ? 'selected' : '' }}>Text</option>
                            <option value="email" {{ $checkoutField->type == 'email' ? 'selected' : '' }}>Email</option>
                            <option value="tel" {{ $checkoutField->type == 'tel' ? 'selected' : '' }}>Telephone</option>
                            <option value="textarea" {{ $checkoutField->type == 'textarea' ? 'selected' : '' }}>Textarea</option>
                            <option value="select" {{ $checkoutField->type == 'select' ? 'selected' : '' }}>Select Dropdown</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="placeholder" value="{{ old('placeholder', $checkoutField->placeholder) }}"
                               class="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-indigo-500 focus:bg-white transition-all outline-none text-slate-900 font-medium"
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="sort_order" required value="{{ old('sort_order', $checkoutField->sort_order) }}"
                               class="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-indigo-500 focus:bg-white transition-all outline-none text-slate-900 font-medium">
                    </div>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <textarea name="options" rows="2"
                                  class="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-indigo-500 focus:bg-white transition-all outline-none text-slate-900 font-medium"
                                  placeholder="e.g. Option 1, Option 2, Option 3">{{ old('options', $checkoutField->options ? implode(', ', $checkoutField->options) : '') }}</textarea>
                    </div>
                    ...
                    <div class="md:col-span-2 flex flex-wrap gap-6">
                        <div class="flex items-center space-x-4 p-4 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_required" value="1" class="sr-only peer" {{ $checkoutField->is_required ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center space-x-4 p-4 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $checkoutField->is_active ? 'checked' : '' }}>
